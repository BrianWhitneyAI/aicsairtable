...
lint:
> $(PYTHON) -m flake8 --count --show-source --statistics aicsairtable
.PHONY: lint

type-check:
> $(PYTHON) -m mypy --ignore-missing-imports  aicsairtable
.PHONY: type-check

fmt:
> $(PYTHON) -m black aicsairtable
.PHONY: fmt

import-sort:
> $(PYTHON) -m isort aicsairtable
.PHONY: import-sort

test:
> $(PYTHON) -m pytest aicsairtable/tests/
.PHONY: test
...
docs:
> source $(ACTIVATE) && sphinx-apidoc -f -o docs aicsairtable aicsairtable/tests
