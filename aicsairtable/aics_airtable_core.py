...
"""
KalebNyquists repo:
https://gist.github.com/KalebNyquist/4424fc1ef3dc6bb4fc0b5a7122ada4bc.js
"""


def airtable_download(
    table, params_dict={}, api_key=None, base_id=None, record_id=None
):
    ...
    if record_id is not None:
        if params_dict is not {}:
            print("⚠️ Caution: parameters are redundant for single record lookups.")
...
def airtable_upload(
    ...
    • typecast: if set to true, Airtable will attempt "best-effort automatic data conversion from string values"
        • see: "Create Records" or "Update Records" in API Documentation.
    • api_key: retrievable at https://airtable.com/account
        ◦ looks like "key●●●●●●●●●●●●●●"
    • base_id: retrievable at https://airtable.com/api for specific base
        ◦ looks like "app●●●●●●●●●●●●●●"
    ...
    if type(upload_data) is not dict:
    ...
    for pandas_dict in pandas_dicts:
        ...
        if (
            validate_airtable_kwargs(
                str(record_id), "Record ID", "rec", print_messages=False
            )
            is False
        ):
            record_id = None
...
def validate_airtable_kwargs(
    ...
    """
    valid_status = True
    if len(kwarg) is not char_length:
